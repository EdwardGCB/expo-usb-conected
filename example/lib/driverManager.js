  ...
  return {
    ...
    process: function (driver, cb) {
      ...
      function uploadBlob(data, next) {
        if(isBrowser && (
          _.isArray(data.pages) ||           // Medtronic 500/700-series and 600-series
          _.isArray(data.aapPackets) ||      // Abbott Libre
          _.isArrayBuffer(data.filedata) ||  // OmniPod & LibreView
          _.isArray(data.packets)            // Tandem
        )) {
          const contentType = 'application/gzip';
          let dataBinary = null;

          console.time('Compressing blob');
          if (data.filedata) {
            dataBinary = new Uint8Array(data.filedata);
          } else {
            let blob = _.omit(data, ['post_records', 'log_records', 'packets']);
            debug('Stringifying blob..');
            dataBinary = stringify(blob);
          }

          if (data.packets) { // Tandem pump
            ...
            data.compressed = deflate.result;
          } else {
            debug('Compressing blob..');
            data.compressed = pako.gzip(dataBinary);
          }

          console.timeEnd('Compressing blob');

          console.time('upload blob elapsed');
          configs[driver].api.upload.blob(data.compressed, contentType, function (err, result) {
            if (err) {
